    ...
    return (
        <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex items-center justify-center'>
            <form onSubmit={onSubmitHandler} className='relative bg-white text-slate-500 rounded-xl p-10 ' >
                <h1 className='text-center text-2xl text-neutral-700 font-md'>Recruiter {state}</h1>
                ...
                <button type='submit' className='bg-blue-600 w-full text-white px-6 py-2 rounded-lg mt-5 hover:bg-blue-700 transition-all duration-200'>
                    {state == 'Login' ? 'Login' : isTextDataSubmitted ? 'create account' : 'next'}
                </button>
                {
                    state === 'Login'
                        ? <p className='text-sm  mt-4 text-center '>Don't have an account? <span className='text-blue-600 cursor-pointer' onClick={e => setState('Sign Up')} >Sign Up</span> </p>
                        :
                        <p className='text-sm  mt-4 text-center '>Already have an account? <span className='text-blue-600 cursor-pointer' onClick={e => setState('Login')}>Login</span></p>
